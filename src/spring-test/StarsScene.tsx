...
import { OrbitControls, Reflector, Stars, Stats } from "@react-three/drei"
...
const hoveredObjectsContext = createContext<
  React.Dispatch<React.SetStateAction<React.MutableRefObject<Object3D>[]>>
>(() => [])
...
function useHover() {
  ...
  const onPointerOut = useCallback(
    () => setHovered(state => state.filter(mesh => mesh !== ref)),
    [],
  )
...
export const Star: FC<StarProps> = ({ ...props }) => {
  ...
  return (
    ...
    >
      <sphereGeometry />
      <meshStandardMaterial color="orange" emissive={new Color(0xffa500)} />
...
export const StarsSceneContent = () => {
  ...
  const focusStar = (vec3: Vector3) => {
    const offsetFromCurrentTarget = camera.position
      .clone()
      .sub(new Vector3(target[0], target[1], target[2]))
    ...
    api.start({
      ...
      onChange: result => {
        ...
        camera.position.set(
          ...
          result.value.position[2],
        )
        camera.lookAt(
          result.value.lookAt[0],
          result.value.lookAt[1],
          result.value.lookAt[2],
        )
      },
    })
  }
