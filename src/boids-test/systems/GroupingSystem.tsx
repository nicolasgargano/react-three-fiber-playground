...
export const GroupingSystem: FC<GroupingSystemProps> = ({ weight }) => {
  ...
  return useSystem(dt => {
    query.loop(
      [ThreeView, Boid, Velocity, Acceleration],
      (currentEntity, [view, boid, { velocity }, { acceleration }]) => {
        if (boid.flockmates.length > 0) {
          ...
          for (const flockmate of boid.flockmates) {
            ...
            if (otherPosition) accumulatedFlockPosition.add(otherPosition)
          }

          const averageFlockPosition = accumulatedFlockPosition.divideScalar(
            boid.flockmates.length,
          )
          const toAverage = averageFlockPosition
            .sub(view.object3d.position)
