...
export const toVec3 = (vector3: Vector3): Vec3 =>
  new Vec3(vector3.x, vector3.y, vector3.z)
export const toVector3 = (vec3: Vec3): Vector3 =>
...
export const generateRandomStartingConditions = (
  ...
  return nonEmptyArray.range(1, settings.amountOfBoids).map(_ => {
    const x = rng.next(-settings.worldSize / 3, settings.worldSize / 3)
    const y = rng.next(-settings.worldSize / 3, settings.worldSize / 3)
    const z = rng.next(-settings.worldSize / 3, settings.worldSize / 3)
